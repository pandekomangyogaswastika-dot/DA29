...
class P2PFlowTester:
    ...
    def test_get_remaining_draft_po(self) -> bool:
        ...
        if success:
            ...
            if total_remaining == 620 and len(items_remaining) == 3:
                self.log(f"   ✓ Draft PO returns full remaining qty")
                return True
            else:
    ...
    def test_get_remaining_approved_po(self) -> bool:
        ...
        if success:
            ...
            self.log(f"   Total remaining: {total_remaining}, Items: {len(items_remaining)}")
            if total_remaining == 620 and len(items_remaining) == 3:
                self.log(f"   ✓ Approved PO returns full remaining qty")
                return True
            else:
                self.log(f"   ⚠ Expected total_remaining=620, got {total_remaining}", "WARN")
                return False
    ...
    def test_create_gr_from_approved_po(self) -> bool:
        ...
        if success:
            ...
            if all_passed:
                self.log(f"   ✓ GR created with correct attributes")
    ...
    def test_verify_po_partially_received(self) -> bool:
        ...
        if success:
            ...
            if status == 'partially_received' and all_correct:
                self.log(f"   ✓ PO status and qty_received correct")
                return True
            else:
                self.log(f"   ⚠ Expected status=partially_received with correct qty_received", "WARN")
                return False
        return False
    ...
    def test_create_gr2_from_partially_received_po(self) -> bool:
        ...
        if success:
            ...
            if all_correct:
                self.log(f"   ✓ GR2 expected_qty = remaining qty")
                return True
            else:
                self.log(f"   ⚠ GR2 expected_qty incorrect", "WARN")
                return False
        return False
    ...
    def test_verify_po_fully_received(self) -> bool:
        ...
        if success:
            ...
            if status == 'fully_received' and all_fully_received:
                self.log(f"   ✓ PO fully received")
                return True
            else:
                self.log(f"   ⚠ Expected status=fully_received with all qty received", "WARN")
                return False
        return False
    ...
    def test_get_grs_audit_trail(self) -> bool:
        ...
        if success and isinstance(response, list):
            ...
            if len(response) == 2:
                self.log(f"   ✓ Audit trail returns 2 GRs")
                return True
            else:
