...
async def create_indexes():
    ...
    try:
        ...
        await db.rahaza_notifications.create_index("user_id")
        await db.rahaza_notifications.create_index([("user_id", 1), ("read", 1)])
        ...
        # Phase 4 P1 — Notifications (mock log)
        await db.dewi_notifications.create_index([("status", 1), ("created_at", -1)])
        await db.dewi_notifications.create_index([("client_id", 1), ("created_at", -1)])
        await db.dewi_notifications.create_index([("event_type", 1), ("source_ref", 1)])
        ...
        await db.dewi_kol_deals.create_index([("creator_id", 1), ("status", 1)])
        await db.dewi_kol_samples.create_index("sample_code", unique=True)
        await db.dewi_kol_samples.create_index([("creator_id", 1), ("status", 1)])
        ...
        # Phase 4: LiveHost portal notifications (SSE persistence)
        await db.marketing_livehost_notifications.create_index("id", unique=True)
        await db.marketing_livehost_notifications.create_index([("host_id", 1), ("created_at", -1)])
        await db.marketing_livehost_notifications.create_index([("host_id", 1), ("read", 1)])
        ...
        # Accessories & Operations — paginated list endpoints
        await db.accessories.create_index([("created_at", -1)])
        await db.accessories.create_index("status")
        ...
        await db.accessory_requests.create_index([("created_at", -1)])
        await db.accessory_requests.create_index("status")

        # Invoices / Payments (Dewi ERP) — list endpoints
        ...
        # APScheduler: retry queued import sessions
        from routes.universal_import import retry_queued_sessions
        scheduler.add_job(retry_queued_sessions, "interval", minutes=5,
...
# from routes.warehouse import router as warehouse_router  # DEPRECATED — use /api/wms/legacy/*
...
from routes.dewi_kol import router as dewi_kol_router
...
from routes.marketing_livehost import router as marketing_livehost_router
...
app.include_router(marketing_livehost_router)
...
app.include_router(dewi_kol_router)
...
# Phase 8 — DA KPI System
from routes.dewi_kpi import router as dewi_kpi_router
app.include_router(dewi_kpi_router)
