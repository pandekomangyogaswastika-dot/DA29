...
async def list_orders(
    ...
    if account_name: q["account_name"] = account_name
    if date_from or date_to:
        q["order_date"] = {}
        if date_from: q["order_date"]["$gte"] = datetime.fromisoformat(date_from)
        if date_to:   q["order_date"]["$lte"] = datetime.fromisoformat(date_to + "T23:59:59")
    if search:
        q["$or"] = [
...
async def update_order_status(
    ...
    if body.status == "packed":    update["packed_date"]    = _now()
    if body.status == "shipped":   update["shipped_date"]   = _now()
    if body.status == "delivered": update["delivered_date"] = _now()
    if body.status == "cancelled": update["cancelled_date"] = _now()
