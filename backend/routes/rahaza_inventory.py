...
async def list_materials(request: Request, type: Optional[str] = None, search: Optional[str] = None,
    ...
    """
    List materials. Pagination: ?page=1&limit=50 → {items, pagination}. Tanpa page → array (legacy).
    """
    ...
    if low_stock and low_stock.lower() == "true":
        # Low-stock filter requires full load + join (in-app calculation)
...
async def list_movements(request: Request, material_id: Optional[str] = None, limit: int = 100):
    """
    List material movements. Pagination: ?page=1&limit=50 → {items, pagination}. Tanpa page → array.
    """
    await require_auth(request)
    db = get_db()
    q = {}
...
async def list_mis(request: Request, work_order_id: Optional[str] = None, status: Optional[str] = None,
                    building_id: Optional[str] = None, limit: int = 200, skip: int = 0):
    await require_auth(request)
    db = get_db()
    q = {}
    ...
    # Multi-warehouse filter — jika building_id diisi, hanya tampilkan MI yang
    # punya pending outbound RM di gedung tersebut (dari wh_pending_movements).
    if building_id:
...
async def draft_mi_from_wo(request: Request):
    ...
    # Prefetch all existing materials in single batch ($in over code)
    ...
    if all_codes:
        async for m in db.rahaza_materials.find(
            {"code": {"$in": all_codes}, "active": True}, {"_id": 0}
        ):
...
async def approve_mi(mid: str, request: Request):
    ...
    # Pre-compute (material_id, location) pairs and batch-fetch stocks in one query
    pairs = set()
    for it in raw_items:
