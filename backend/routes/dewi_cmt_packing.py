...
async def list_receipts(request: Request):
    ...
    if sp.get("cmt_name"): query["cmt_name"] = re.compile(re.escape(sp["cmt_name"]), re.IGNORECASE)
    if sp.get("wo_number"):query["wo_number"]= re.compile(re.escape(sp["wo_number"]), re.IGNORECASE)
