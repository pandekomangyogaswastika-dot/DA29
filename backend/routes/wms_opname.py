...
router = APIRouter(prefix="/api/wms", tags=["wms-opname"])
...
async def _next_opname_ref(db) -> str:
    res = await db.wh_counters.find_one_and_update(
        {"_id": "OPN"}, {"$inc": {"seq": 1}}, upsert=True, return_document=True
    )
    return f"OPN-{res.get('seq', 1):05d}"
...
async def opname_detail(session_id: str, request: Request):
    ...
    for l in scanned:
        ...
        total_diff += diff
        if diff > 0: pos_diff += 1
        elif diff < 0: neg_diff += 1
