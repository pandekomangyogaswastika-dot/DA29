...
async def my_notifications(request: Request, skip: int = 0, limit: int = 30):
    ...
    db = get_db()
    uid = user["id"]
    q = {"$or": [{"user_id": uid}, {"user_ids": uid}]}
    total = await db.rahaza_notifications.count_documents(q)
    rows = await db.rahaza_notifications.find(q, {"_id": 0}).sort("created_at", -1).skip(skip).limit(limit).to_list(500)
...
async def mark_read(notif_id: str, request: Request):
    ...
    await db.rahaza_notifications.update_one(
        {"id": notif_id}, {"$set": {"read": True, "read_at": _now_iso()}}
    )
    return {"ok": True}
