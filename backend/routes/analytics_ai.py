...
async def production_rca(data: RCAFilter, request: Request):
    ...
    if data.line_id: wip_q["line_id"] = data.line_id
    if data.model_id: wip_q["model_id"] = data.model_id
    if data.process_code: wip_q["process_code"] = data.process_code
    ...
    # Build prompt
    filters_desc = []
    if data.line_id: filters_desc.append(f"line={data.line_id}")
    if data.model_id: filters_desc.append(f"model={data.model_id}")
    if data.process_code: filters_desc.append(f"process={data.process_code}")
...
async def qc_rca(data: RCAFilter, request: Request):
    ...
    if data.line_id: q["line_id"] = data.line_id
    if data.model_id: q["model_id"] = data.model_id
