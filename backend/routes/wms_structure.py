...
async def update_building(building_id: str, data: BuildingIn, request: Request):
    ...
    if not bldg: raise HTTPException(404, "Gedung tidak ditemukan")
...
async def list_zones(request: Request, building_id: Optional[str] = None):
    ...
    db = get_db()
    q = {"active": True}
    if building_id: q["building_id"] = building_id
...
async def create_zone(data: ZoneIn, request: Request):
    ...
    if not building: raise HTTPException(404, "Gedung tidak ditemukan")
    code = data.code.upper().strip()
...
async def update_zone(zone_id: str, data: ZoneIn, request: Request):
    ...
    if not zone: raise HTTPException(404, "Zona tidak ditemukan")
...
async def update_rack(rack_id: str, data: RackIn, request: Request):
    ...
    if not rack: raise HTTPException(404, "Rak tidak ditemukan")
...
async def warehouse_search(
    ...
    if building_id: pos_q["building_id"] = building_id
