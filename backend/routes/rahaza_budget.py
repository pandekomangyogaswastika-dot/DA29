...
async def add_budget_item(bid: str, request: Request):
    ...
    db = get_db()
    budget = await db.rahaza_budgets.find_one({"id": bid}, {"_id": 0})
    if not budget: raise HTTPException(404, "Budget tidak ditemukan")
    if budget.get("status") == "locked": raise HTTPException(409, "Budget terkunci")
    body = await request.json()
...
async def update_budget_item(bid: str, iid: str, request: Request):
    ...
    if "amount_budgeted" in body:
        try: update["amount_budgeted"] = float(body["amount_budgeted"])
        except (ValueError, TypeError): raise HTTPException(400, "amount_budgeted harus berupa angka")
    if "notes" in body: update["notes"] = (body["notes"] or "").strip()
    if "month" in body: update["month"] = body["month"]
    if "cost_center_id" in body: update["cost_center_id"] = body["cost_center_id"]
    if update:
        update["updated_at"] = _now()
...
async def budget_variance(bid: str, request: Request):
    ...
    budget = await db.rahaza_budgets.find_one({"id": bid}, {"_id": 0})
    if not budget: raise HTTPException(404, "Budget tidak ditemukan")
    year = budget.get("year", date.today().year)
