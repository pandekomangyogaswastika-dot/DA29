...
async def create_asset(request: Request):
    ...
    if not name: raise HTTPException(400, "asset_name wajib diisi.")
...
async def update_asset(asset_id: str, request: Request):
    ...
    if res.matched_count == 0: raise HTTPException(404, "Aset tidak ditemukan.")
...
async def delete_asset(asset_id: str, request: Request):
    ...
    if active: raise HTTPException(400, "Aset masih sedang ditugaskan. Return dulu sebelum dihapus.")
...
async def assign_asset(asset_id: str, request: Request):
    ...
    if not emp: raise HTTPException(404, "Karyawan tidak ditemukan.")
...
async def return_asset(asset_id: str, request: Request):
    ...
    if not active: raise HTTPException(404, "Tidak ada penugasan aktif untuk aset ini.")
...
async def print_bulk_labels(request: Request, category: Optional[str] = None):
    ...
    if not assets: raise HTTPException(404, "Tidak ada aset ditemukan.")
