...
async def create_material_issue(order_id: str, payload: MaklonMaterialIssueIn, user: dict = Depends(require_auth)):
    ...
    loc_doc = await db.warehouse_locations.find_one({'id': payload.location_id}, {'_id': 0})
    loc_name = loc_doc.get('name', payload.location_id) if loc_doc else payload.location_id
