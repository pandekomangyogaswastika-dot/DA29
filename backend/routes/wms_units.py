...
async def create_conversion(data: ConversionIn, request: Request):
    ...
    if not fu: raise HTTPException(404, f"Unit '{data.from_unit_code}' tidak ditemukan")
    if not tu: raise HTTPException(404, f"Unit '{data.to_unit_code}' tidak ditemukan")
...
async def update_conversion(conv_id: str, data: ConversionIn, request: Request):
    ...
    if not conv: raise HTTPException(404, "Konversi tidak ditemukan")
...
async def convert_qty(data: ConvertCalcIn, request: Request):
    ...
    if not fu: raise HTTPException(404, f"Unit '{data.from_unit}' tidak ditemukan")
    if not tu: raise HTTPException(404, f"Unit '{data.to_unit}' tidak ditemukan")
