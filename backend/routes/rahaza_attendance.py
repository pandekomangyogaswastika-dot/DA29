...
async def get_office_location(request: Request):
    ...
    db = get_db()
    doc = await db.rahaza_office_locations.find_one({"is_primary": True}, {"_id": 0})
    return doc or {"is_primary": True, "name": "Kantor Utama", "lat": None, "lng": None, "geofence_radius_m": 300}
...
async def set_office_location(request: Request):
    ...
    doc = {
        ...
        "geofence_radius_m": int(body.get("geofence_radius_m") or 300),
        "updated_at": _now(),
    }
...
async def clock_in(request: Request):
    ...
    # ─── P2 Geolocation validation ───────────────────────────────────────
    lat = body.get("lat")
    lng = body.get("lng")
    geo_status = "not_verified"
    geo_distance_m = None
    if lat is not None and lng is not None:
        # Check against configured office coordinates
        office = await db.rahaza_office_locations.find_one({"is_primary": True}, {"_id": 0})
        if office:
            import math
            # Haversine
            R = 6371000  # m
            lat1, lon1 = math.radians(float(office["lat"])), math.radians(float(office["lng"]))
            lat2, lon2 = math.radians(float(lat)), math.radians(float(lng))
            dlat, dlon = lat2 - lat1, lon2 - lon1
            a = math.sin(dlat/2)**2 + math.cos(lat1)*math.cos(lat2)*math.sin(dlon/2)**2
            dist = 2 * R * math.asin(math.sqrt(a))
            geo_distance_m = round(dist)
            radius = float(office.get("geofence_radius_m", 300))
            geo_status = "in_range" if dist <= radius else "out_of_range"
