...
async def get_unit(
    ...
    if not doc: raise HTTPException(404, "Unit tidak ditemukan")
    return {"ok": True, "unit": serialize(doc)}
...
async def list_positions(
    ...
    filt = {}
    if unit_id: filt["unit_id"] = unit_id
...
async def seed_org(
    ...
    for u in units: await db.dewi_org_units.insert_one(u)
    ...
    for p in positions: await db.dewi_org_positions.insert_one(p)
