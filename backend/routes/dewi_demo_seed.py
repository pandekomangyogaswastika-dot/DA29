...
async def seed_phase3_maklon(db, user_name: str) -> dict:
    ...
    await db.dewi_maklon_orders.insert_many(order_seed)
...
async def seed_phase5_toko(db, user_name: str) -> dict:
    ...
    # --- KOL CREATORS (if less than 4) ---
    if await db.dewi_kol_creators.count_documents({}) < 4:
        creator_seed = [
            {"id": _uid(), "name": "Tasya Permata", "phone": "081200000001",
             "address": "Jl. Senayan", "city": "Jakarta", "channel_type": "tiktok_shop",
             "category": "power_partner", "username": "@tasyaperm", "followers": 850000,
             "status": "active", "notes": "Top seller conversion 8%",
             "created_at": now - timedelta(days=90), "updated_at": now, "created_by": user_name},
            {"id": _uid(), "name": "Rani Andini", "phone": "081200000002",
             "address": "Jl. Pakuwon", "city": "Surabaya", "channel_type": "instagram",
             "category": "viral_maker", "username": "@ranistyle", "followers": 320000,
             "status": "active", "notes": "Spesialis video viral casual",
             "created_at": now - timedelta(days=60), "updated_at": now, "created_by": user_name},
            {"id": _uid(), "name": "Shinta Kirana", "phone": "081200000003",
             "address": "Jl. Dago", "city": "Bandung", "channel_type": "tiktok_shop",
             "category": "potential", "username": "@shintkirana", "followers": 95000,
             "status": "active", "notes": "Baru join, growing fast",
             "created_at": now - timedelta(days=30), "updated_at": now, "created_by": user_name},
            {"id": _uid(), "name": "Dinda Maharani", "phone": "081200000004",
             "address": "Jl. Malioboro", "city": "Yogyakarta", "channel_type": "shopee",
             "category": "active", "username": "@dindamah", "followers": 50000,
             "status": "active", "notes": "Live stream Shopee tiap Jumat",
             "created_at": now - timedelta(days=20), "updated_at": now, "created_by": user_name},
            {"id": _uid(), "name": "Ayu Wulandari", "phone": "081200000005",
             "address": "Jl. Asia Afrika", "city": "Bandung", "channel_type": "youtube",
             "category": "passive", "username": "@ayuwulandari", "followers": 180000,
             "status": "inactive", "notes": "Pause content sampai Juni",
             "created_at": now - timedelta(days=120), "updated_at": now, "created_by": user_name},
        ]
        await db.dewi_kol_creators.insert_many(creator_seed)
        counts["creators"] = len(creator_seed)

    # --- KOL DEALS (if less than 3) ---
    if await db.dewi_kol_deals.count_documents({}) < 3:
        creators = await db.dewi_kol_creators.find({"status": "active"}, {"id": 1, "name": 1, "_id": 0}).to_list(length=5)
        skus_for_deal = await db.dewi_toko_products.find({}, {"sku_code": 1, "name": 1, "_id": 0}).limit(3).to_list(length=3)
        if creators and skus_for_deal:
            deal_types = ["tiktok_video", "live_stream", "feed_post", "review", "story"]
            deal_statuses = ["active", "completed", "active", "draft"]
            existing_count = await db.dewi_kol_deals.count_documents({})
            deal_seed = []
            for i, c in enumerate(creators[:4]):
                deal_seed.append({
                    "id": _uid(),
                    "deal_code": f"DEAL-20260501-{str(existing_count + i + 1).zfill(3)}",
                    "creator_id": c["id"], "creator_name": c["name"],
                    "products": [{"sku_code": s["sku_code"], "product_name": s["name"]} for s in skus_for_deal[:2]],
                    "deal_type": deal_types[i % len(deal_types)],
                    "commission_pct": 10 + (i * 2.5),
                    "notes": f"Deal dengan {c['name']} untuk campaign bulan ini",
                    "start_date": _date_str(-10 + i * 2), "end_date": _date_str(20 + i * 2),
                    "status": deal_statuses[i % len(deal_statuses)],
                    "result_notes": "Conversion bagus" if deal_statuses[i % len(deal_statuses)] == "completed" else None,
                    "created_at": now - timedelta(days=15 - i), "updated_at": now - timedelta(days=1),
                    "created_by": user_name,
                })
            await db.dewi_kol_deals.insert_many(deal_seed)
            counts["deals"] = len(deal_seed)

    # --- KOL SAMPLES (if less than 3) ---
    if await db.dewi_kol_samples.count_documents({}) < 3:
        creators = await db.dewi_kol_creators.find({"status": "active"}, {"id": 1, "name": 1, "_id": 0}).to_list(length=5)
        deals = await db.dewi_kol_deals.find({}, {"id": 1, "_id": 0}).to_list(length=5)
        skus_for_sample = await db.dewi_toko_products.find({}, {"sku_code": 1, "name": 1, "_id": 0}).limit(3).to_list(length=3)
        if creators and skus_for_sample:
            sample_statuses = ["shipped", "received", "feedback", "done"]
            existing_count = await db.dewi_kol_samples.count_documents({})
            sample_seed = []
            for i, c in enumerate(creators[:4]):
                s = skus_for_sample[i % len(skus_for_sample)]
                status = sample_statuses[i % len(sample_statuses)]
                deal_id = deals[i % len(deals)]["id"] if i < len(deals) else None
                sample_seed.append({
                    "id": _uid(),
                    "sample_code": f"SMPL-20260501-{str(existing_count + i + 1).zfill(3)}",
                    "creator_id": c["id"], "creator_name": c["name"],
                    "deal_id": deal_id,
                    "sku_code": s["sku_code"], "product_name": s["name"],
                    "qty": 2, "notes": f"Sample untuk {c['name']}",
                    "status": status,
                    "tracking_number": f"SAMPL{10000 + i}" if status != "draft" else None,
                    "courier": "JNE" if status != "draft" else None,
                    "sent_at": (now - timedelta(days=7 - i)).isoformat() if status != "draft" else None,
                    "received_at": (now - timedelta(days=4 - i)).isoformat() if status in ("received", "feedback", "done") else None,
                    "feedback_notes": "Produk bagus, cocok untuk konten" if status in ("feedback", "done") else None,
                    "created_at": now - timedelta(days=8 - i), "updated_at": now - timedelta(days=1),
                    "created_by": user_name,
                })
            await db.dewi_kol_samples.insert_many(sample_seed)
            counts["samples"] = len(sample_seed)
