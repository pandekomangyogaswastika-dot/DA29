...
async def list_jobs(
    ...
    if dept: filt["department"] = dept
    if q: filt["$or"] = [{"title": {"$regex": re.escape(q), "$options": "i"}},
...
async def get_job(
    ...
    if not doc: raise HTTPException(404, "Lowongan tidak ditemukan")
    s = serialize(doc)
...
async def list_candidates(
    ...
    filt = {}
    if job_id: filt["job_id"] = job_id
    if stage: filt["stage"] = stage
    if q: filt["$or"] = [{"name": {"$regex": re.escape(q), "$options": "i"}},
...
async def add_candidate(
    ...
    if job_id:
        ...
        if not job: raise HTTPException(404, "Lowongan tidak ditemukan")
...
async def get_candidate(
    ...
    if not doc: raise HTTPException(404, "Kandidat tidak ditemukan")
    return {"ok": True, "candidate": serialize(doc)}
...
async def update_candidate(
    ...
    if not candidate: raise HTTPException(404)
    ...
    if "stage" in body and body["stage"] != candidate.get("stage"):
        ...
        if new_stage == "Hired" and candidate.get("job_id"):
            ...
            try:
                ...
                import logging; logging.getLogger(__name__).error(f"Auto-onboarding failed: {e}")
...
async def seed_recruitment(
    ...
    for j in jobs: await db.dewi_recruitment_jobs.insert_one(j)
