...
async def _next_ref(db, prefix: str) -> str:
    result = await db.wh_counters.find_one_and_update(
        {"_id": prefix},
        {"$inc": {"seq": 1}},
        upsert=True,
...
async def pending_summary(request: Request, building_id: Optional[str] = None):
    ...
    if building_id: q_base["building_id"] = building_id
...
async def scan_in(movement_id: str, data: ScanInIn, request: Request):
    ...
    if not movement: raise HTTPException(404, "Pending movement tidak ditemukan")
