...
async def queue_notification(
    ...
    *,
    ...
    """
    Insert notification record and attempt real sending if provider configured.
    Returns notif id.
    """
...
async def list_notifications(
    ...
    rows = await db.dewi_notifications.find(q, {'_id': 0}).sort('created_at', -1).limit(limit).to_list(500)
    return rows
...
async def summary(user: dict = Depends(require_auth)):
    db = get_db()
    total = await db.dewi_notifications.count_documents({})
    by_status = {
        'queued':  await db.dewi_notifications.count_documents({'status': 'queued'}),
        'sent':    await db.dewi_notifications.count_documents({'status': 'sent'}),
        'failed':  await db.dewi_notifications.count_documents({'status': 'failed'}),
    }
    by_channel = {
        'whatsapp': await db.dewi_notifications.count_documents({'channel': 'whatsapp'}),
        'email':    await db.dewi_notifications.count_documents({'channel': 'email'}),
    }
    config = await _get_provider_config(db)
    return {
        'total': total,
...
async def bulk_send_queued(user: dict = Depends(require_auth)):
    ...
    db = get_db()
    config = await _get_provider_config(db)
    queued = await db.dewi_notifications.find({'status': 'queued'}, {'_id': 0}).to_list(500)
    ...
    for notif in queued:
        ...
        body = notif.get('body', '')
        subject = notif.get('subject', 'CV. Dewi Aditya')
        ...
        if ok:
            await db.dewi_notifications.update_one(
                {'id': notif['id']},
                {'$set': {'status': 'sent', 'sent_at': _now(), 'sent_real': True, 'sent_mock': False}}
            )
            results['sent'] += 1
...
async def mark_sent(notif_id: str, user: dict = Depends(require_auth)):
    ...
    db = get_db()
    notif = await db.dewi_notifications.find_one({'id': notif_id})
    if not notif:
    ...
    if notif['channel'] == 'whatsapp':
        ...
        ok, err = await _try_send_email(
            notif['recipient'], notif.get('subject', 'CV. Dewi Aditya'), notif['body'], config
        )

    if ok:
        update = {'status': 'sent', 'sent_at': _now(), 'sent_real': True, 'sent_mock': False}
    else:
        # MOCK send
        update = {'status': 'sent', 'sent_at': _now(), 'sent_real': False, 'sent_mock': True,
                  'failed_reason': err}

    await db.dewi_notifications.update_one({'id': notif_id}, {'$set': update})
    return {
...
async def delete_notif(notif_id: str, user: dict = Depends(require_auth)):
    db = get_db()
    res = await db.dewi_notifications.delete_one({'id': notif_id})
...
async def scan_overdue(user: dict = Depends(require_auth)):
    ...
    if inv_ids:
        async for n in db.dewi_notifications.find(
            {'event_type': 'invoice_overdue', 'source_ref': {'$in': inv_ids}},
            {'_id': 0, 'source_ref': 1}
        ):
