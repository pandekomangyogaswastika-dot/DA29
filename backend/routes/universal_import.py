...
async def _ai_detect_schema(text_content: str, filename: str, file_type: str, is_image: bool = False) -> dict:
    ...
    if is_image:
        user_msg = UserMessage(
            text=(
                f"This is a screenshot from an Indonesian marketplace (Shopee/TikTok) dashboard.\n"
                f"Extract all visible metrics and return JSON with structure:\n"
                f'{{"detected_source":"account_health","platform":"shopee or tiktok",'
                f'"overall_confidence":0.9,"warnings":[],"ocr_data":{{}},"column_mappings":[]}}\n'
                f"Put extracted metrics in ocr_data as key-value pairs."
            ),
