...
async def seed_product_launches_if_empty():
    ...
    for i, prod in enumerate(products):
        ...
        entries.append({
            ...
            "platforms":         plats,
            "description":       f"Produk fashion muslimah berkualitas untuk koleksi 2026.",
            "status":            status,
...
async def list_launches(
    ...
    if search:    q["product_name"] = {"$regex": search, "$options": "i"}
    if date_from: q.setdefault("launch_date", {})["$gte"] = date_from
    if date_to:   q.setdefault("launch_date", {})["$lte"] = date_to
