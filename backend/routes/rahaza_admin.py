...
async def _seed_coa(db, user: dict):
    ...
    # Batch fetch existing COA accounts in single query
    existing_codes = set()
...
async def seed_demo_data(request: Request):
    ...
    # Prefetch all customers & models once to avoid N+1 inside loop
    customer_map = {}
    ...
    payroll_count = 0
    # Batch-fetch all existing run_numbers for the 3-month window
    pr_run_numbers = []
    ...
    for month_off in range(3, 0, -1):
        ...
        # Skip if exists (from prefetched set)
    ...
    # Batch fetch orders for completed WOs
    ...
    # ── 12. WIP Events (output events from line assignments) ───────────────
    wip_count = 0
    ...
    line_map = {l["id"]: l for l in await db.rahaza_lines.find({}, {"_id": 0}).to_list(500)}
    ...
    for assignment in all_assignments:
        ...
        for emp_id in emp_ids:
            ...
            ev = {
                ...
                "event_date": assign_date,                # date string for queries
                "line_id": line_id,
