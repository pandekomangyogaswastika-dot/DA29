...
async def create_cycle(request: Request):
    ...
    if not target_id: raise HTTPException(400, "target_employee_id wajib.")

    target = await db.rahaza_employees.find_one({"id": target_id}, {"_id": 0})
    if not target: raise HTTPException(404, "Target karyawan tidak ditemukan.")
...
async def list_cycles(request: Request, status: Optional[str] = None,
    ...
    if target_id: filt["target_employee_id"] = target_id
...
async def get_cycle(cycle_id: str, request: Request):
    ...
    if not doc: raise HTTPException(404, "Cycle tidak ditemukan.")
    return {"ok": True, "cycle": _s(doc)}
...
async def submit_feedback(cycle_id: str, request: Request):
    ...
    if not cycle: raise HTTPException(404, "Cycle tidak ditemukan.")
    if cycle.get("status") != "open": raise HTTPException(400, "Cycle sudah ditutup.")
...
async def close_cycle(cycle_id: str, request: Request):
    ...
    for q in cycle.get("questions", []):
        if q.get("type") == "text": continue
        qid = q["id"]
        scores = []
        by_relation = {}
        for r in cycle.get("reviewers", []):
            if r.get("status") != "submitted": continue
