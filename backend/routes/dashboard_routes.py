...
from routes.shared import new_id, now, parse_date, to_end_of_day, PO_STATUSES, enrich_with_product_photos, get_pagination_params, paginated_response
import uuid
import json
import logging
...
async def get_dashboard(request: Request):
    ...
    # ── Revenue Today / MTD / YTD (from rahaza AR invoices + plain invoices) ──
    ...
    # Fallback to plain invoices collection
    plain_invs = await db.invoices.find(
        {}, {'_id': 0, 'amount': 1, 'total': 1, 'created_at': 1}
    ).to_list(500)
    revenue_today += sum(i.get('total', i.get('amount', 0)) for i in plain_invs
                         if i.get('created_at') and _in_range(i['created_at'], today_start, today_end))
    revenue_mtd   += sum(i.get('total', i.get('amount', 0)) for i in plain_invs
                         if i.get('created_at') and _in_range(i['created_at'], mtd_start, today_end))
    revenue_ytd   += sum(i.get('total', i.get('amount', 0)) for i in plain_invs
                         if i.get('created_at') and _in_range(i['created_at'], ytd_start, today_end))
