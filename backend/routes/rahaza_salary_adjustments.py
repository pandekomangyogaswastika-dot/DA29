...
async def _notify_manager_approval_needed(db, adjustment: dict):
    """Send notification to manager when new raise proposal is created"""
    ...
    # Create notification (assuming rahaza_notifications collection exists)
    await db.rahaza_notifications.insert_one({
        "id": _uid(),
        "user_id": mgr_emp["user_id"],
        "type": "salary_raise_approval",
        "title": "Approval Kenaikan Gaji Diperlukan",
        "message": f"Proposal kenaikan gaji untuk {adjustment.get('employee_name')} ({adjustment.get('raise_pct')}%) menunggu approval Anda.",
        "action_url": f"/hr/salary-adjustments/{adjustment['id']}",
        "read": False,
        "created_at": _now(),
    })
...
async def _notify_hr_approval_needed(db, adjustment: dict):
    """Send notification to HR when manager has approved"""
    # Find all HR users
    ...
    for user in hr_users:
        await db.rahaza_notifications.insert_one({
            "id": _uid(),
            "user_id": user["id"],
            "type": "salary_raise_hr_approval",
            "title": "Approval HR: Kenaikan Gaji",
            "message": f"Proposal kenaikan gaji {adjustment.get('employee_name')} ({adjustment.get('raise_pct')}%) sudah disetujui manager. Menunggu approval HR.",
            "action_url": f"/hr/salary-adjustments/{adjustment['id']}",
            "read": False,
            "created_at": _now(),
        })
...
async def _notify_employee_raise_approved(db, adjustment: dict):
    """Send notification to employee when raise is approved"""
    ...
    await db.rahaza_notifications.insert_one({
        "id": _uid(),
        "user_id": emp["user_id"],
        "type": "salary_raise_approved",
        "title": "Selamat! Kenaikan Gaji Disetujui",
        "message": f"Proposal kenaikan gaji Anda sebesar {adjustment.get('raise_pct')}% telah disetujui. Efektif mulai periode payroll berikutnya.",
