...
from routes.shared import new_id, now, parse_date, to_end_of_day, PO_STATUSES, enrich_with_product_photos, _fmt_date, _fmt_num, _fmt_money, get_pagination_params, paginated_response
import uuid
import json
import logging
...
async def get_accessories(request: Request):
    ...
    query = {}
    if sp.get('search'):
        query['$or'] = [{'name': {'$regex': re.escape(sp['search']), '$options': 'i'}},
                        {'code': {'$regex': re.escape(sp['search']), '$options': 'i'}}]
    if sp.get('status'): query['status'] = sp['status']
    # Support both paginated and full-list modes
    if sp.get('page') or sp.get('limit'):
        ...
        items = await db.accessories.find(query, {'_id': 0}).sort('created_at', -1).skip(skip).limit(limit).to_list(limit)
        return paginated_response(serialize_doc(items), total, page, limit)
    return serialize_doc(await db.accessories.find(query, {'_id': 0}).sort('created_at', -1).to_list(500))
...
async def get_accessory_shipments(request: Request):
    ...
    async def _enrich(shipments):
        """Batch-prefetch all shipment items — eliminates N+1."""
        if not shipments:
            return []
        ship_ids = [s['id'] for s in shipments]
        raw_items = await db.accessory_shipment_items.find({'shipment_id': {'$in': ship_ids}}, {'_id': 0}).to_list(5000)
        items_by_ship = {}
        for it in raw_items:
            items_by_ship.setdefault(it['shipment_id'], []).append(it)
        return [{**serialize_doc(s), 'items': serialize_doc(items_by_ship.get(s['id'], []))} for s in shipments]

    # Support both paginated and full-list modes
    if sp.get('page') or sp.get('limit'):
        page, limit, skip = get_pagination_params(request, default_limit=50)
        total = await db.accessory_shipments.count_documents(query)
        shipments = await db.accessory_shipments.find(query, {'_id': 0}).sort('created_at', -1).skip(skip).limit(limit).to_list(limit)
        return paginated_response(await _enrich(shipments), total, page, limit)
    shipments = await db.accessory_shipments.find(query, {'_id': 0}).sort('created_at', -1).to_list(500)
    return await _enrich(shipments)
...
async def get_acc_inspections(request: Request):
    ...
    if sp.get('page') or sp.get('limit'):
        page, limit, skip = get_pagination_params(request, default_limit=50)
        total = await db.accessory_inspections.count_documents(query)
        items = await db.accessory_inspections.find(query, {'_id': 0}).sort('created_at', -1).skip(skip).limit(limit).to_list(limit)
        return paginated_response(serialize_doc(items), total, page, limit)
    return serialize_doc(await db.accessory_inspections.find(query, {'_id': 0}).sort('created_at', -1).to_list(500))
...
async def create_acc_inspection(request: Request):
    ...
    items_docs = await db.accessory_inspection_items.find({'inspection_id': insp_id}, {'_id': 0}).to_list(500)
...
async def get_acc_defects(request: Request):
    ...
    if sp.get('page') or sp.get('limit'):
        page, limit, skip = get_pagination_params(request, default_limit=50)
        total = await db.accessory_defects.count_documents(query)
        items = await db.accessory_defects.find(query, {'_id': 0}).sort('created_at', -1).skip(skip).limit(limit).to_list(limit)
        return paginated_response(serialize_doc(items), total, page, limit)
    return serialize_doc(await db.accessory_defects.find(query, {'_id': 0}).sort('created_at', -1).to_list(500))
...
async def get_acc_requests(request: Request):
    ...
    if sp.get('page') or sp.get('limit'):
        page, limit, skip = get_pagination_params(request, default_limit=50)
        total = await db.accessory_requests.count_documents(query)
        items = await db.accessory_requests.find(query, {'_id': 0}).sort('created_at', -1).skip(skip).limit(limit).to_list(limit)
        return paginated_response(serialize_doc(items), total, page, limit)
    return serialize_doc(await db.accessory_requests.find(query, {'_id': 0}).sort('created_at', -1).to_list(500))
...
async def export_excel(request: Request):
    ...
    try:
        ...
        if export_type == 'production-pos':
            ...
            ws.append(headers)
            invs = await db.invoices.find({}, {'_id': 0}).sort('created_at', -1).to_list(500)
            ...
            ws.append(headers)
            accs = await db.accessories.find({}, {'_id': 0}).sort('created_at', -1).to_list(500)
