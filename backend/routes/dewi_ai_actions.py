...
async def create_action(request: Request):
    ...
    if not title: raise HTTPException(400, "title wajib diisi.")
...
async def update_action(action_id: str, request: Request):
    ...
    if upd.get("assignee_id") and not upd.get("assignee_name"):
        ...
        if emp: upd["assignee_name"] = emp.get("name")
    ...
    if res.matched_count == 0: raise HTTPException(404, "Action tidak ditemukan.")
...
async def delete_action(action_id: str, request: Request):
    ...
    if res.deleted_count == 0: raise HTTPException(404, "Action tidak ditemukan.")
    return {"ok": True}
