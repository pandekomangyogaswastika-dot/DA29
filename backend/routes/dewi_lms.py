...
async def get_course(
    ...
    if not doc: raise HTTPException(404, "Course tidak ditemukan")
    # attach materials
...
async def list_enrollments(
    ...
    if employee_id: filt["employee_id"] = employee_id
    if status: filt["status"] = status
...
async def update_progress(
    ...
    upd = {"updated_at": now_utc()}
    if "progress_pct" in body: upd["progress_pct"] = body["progress_pct"]
    if "completed_materials" in body: upd["completed_materials"] = body["completed_materials"]
    if "status" in body: upd["status"] = body["status"]
    if "quiz_score" in body: upd["quiz_score"] = body["quiz_score"]
    ...
    if body.get("progress_pct", 0) >= 100 or body.get("status") == "completed":
        ...
        if passed and not enroll.get("certificate_issued"):
            ...
            try:
                ...
                import logging; logging.getLogger(__name__).warning(f"LMS notif failed: {ne}")
