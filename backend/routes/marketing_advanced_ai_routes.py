...
"""
import uuid, logging, os, json, math
...
def ok(data=None, meta=None):
    ...
    if meta is not None: r["metadata"] = meta
    return r

def serialize(o):
    if isinstance(o, list):     return [serialize(i) for i in o]
    if isinstance(o, dict):     return {k: serialize(v) for k, v in o.items() if k != "_id"}
...
async def list_pricing_suggestions(
    ...
    if direction: filt["direction"] = direction
...
async def approve_suggestion(suggestion_id: str, request: Request):
    ...
    if not doc: raise HTTPException(404, "Suggestion tidak ditemukan")
    if doc["status"] != "pending": raise HTTPException(400, f"Status sudah {doc['status']}")
...
async def reject_suggestion(
    ...
    doc = await db.marketing_dynamic_pricing_suggestions.find_one({"id": suggestion_id})
    if not doc: raise HTTPException(404, "Suggestion tidak ditemukan")
    if doc["status"] not in ["pending", "approved"]: raise HTTPException(400, f"Status sudah {doc['status']}")
    await db.marketing_dynamic_pricing_suggestions.update_one(
        {"id": suggestion_id},
...
async def run_churn_analysis(request: Request):
    ...
    async for c in db.marketing_orders.aggregate(pipeline):
        ...
        # Churn risk
        if rfm >= 12:   risk = "low"
        elif rfm >= 8:  risk = "medium"
        elif rfm >= 5:  risk = "high"
        else:           risk = "critical"
...
async def get_ab_test(exp_id: str, request: Request):
    ...
    if not doc: raise HTTPException(404, "Experiment tidak ditemukan")
    return ok(data=serialize(doc))
...
async def record_ab_result(exp_id: str, payload: ABResultIn, request: Request):
    ...
    if not updated: raise HTTPException(404, "Variant tidak ditemukan")
    await db.marketing_ab_experiments.update_one(
...
async def conclude_ab_test(exp_id: str, request: Request):
    ...
    doc = await db.marketing_ab_experiments.find_one({"id": exp_id})
    if not doc: raise HTTPException(404, "Experiment tidak ditemukan")
    if not EMERGENT_LLM_KEY: raise HTTPException(503, "AI tidak dikonfigurasi")
...
async def update_variant_content(exp_id: str, variant_id: str, request: Request):
    ...
    for v in variants:
        if v["id"] == variant_id:
            if "label" in body:   v["label"] = body["label"]
            if "content" in body: v["content"] = body["content"]
    await db.marketing_ab_experiments.update_one(
        {"id": exp_id}, {"$set": {"variants": variants, "updated_at": _now()}}
    )
