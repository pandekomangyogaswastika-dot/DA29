...
async def list_rolls(
    ...
    if material_code: q["material_code"] = {"$regex": material_code, "$options": "i"}
    if color: q["color"] = {"$regex": color, "$options": "i"}
    if color_lot: q["color_lot"] = {"$regex": color_lot, "$options": "i"}
    if status: q["status"] = status
    if qc_status: q["qc_status"] = qc_status
    if position_id: q["position_id"] = position_id
    if search:
        q["$or"] = [
...
async def get_roll(roll_id: str, request: Request):
    ...
    if not roll: raise HTTPException(404, "Roll tidak ditemukan")
...
async def update_roll(roll_id: str, data: dict, request: Request):
    ...
    if not updates: raise HTTPException(400, "Tidak ada field yang dapat diupdate")
