...
- Selalu berdasarkan data yang diberikan, jangan karang data"""
    ...
    ).sort("created_at", 1).limit(20).to_list(500)
...
async def ai_smart_search(request: Request):
    ...
    # Search WOs
    wos = await db.rahaza_work_orders.find({}, {"_id": 0}).limit(500).to_list(500)
    for wo in wos:
        ...
        match = (
            ...
            ("aktif" in q_lower and status in ["in_progress", "released"]) or
    ...
    # Search orders
    orders = await db.rahaza_orders.find({}, {"_id": 0}).limit(200).to_list(500)
    for o in orders:
    ...
    # Search employees
    emps = await db.rahaza_employees.find({"active": True}, {"_id": 0}).limit(100).to_list(500)
    for e in emps:
...
async def predictive_delay(
    ...
    if wo_id:
        wos = await db.rahaza_work_orders.find({"id": wo_id}, {"_id": 0}).to_list(500)
    else:
        wos = await db.rahaza_work_orders.find(
            {"status": {"$in": ["released", "in_progress"]}}, {"_id": 0}
        ).limit(50).to_list(500)
