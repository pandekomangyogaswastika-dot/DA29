"""
WMS — Enhanced Stock Opname (P1-WH-2)
...
Prefix: /api/wms/opname2 (additive — tidak menggantikan wms_opname.py agar backward compat)
...
async def _load_scope_positions(db, scope_type: str, scope_id: str) -> list:
    ...
    q = {}
    if scope_type == "building" and scope_id: q["building_id"] = scope_id
    elif scope_type == "zone" and scope_id: q["zone_id"] = scope_id
    elif scope_type == "rack" and scope_id: q["rack_id"] = scope_id
...
async def start_opname(data: StartOpnameIn, request: Request):
    ...
    open_count = await db.wh_opname_sessions2.count_documents({"status": "open"})
    if open_count > 0:
...
async def scan_count(session_id: str, data: ScanCountIn, request: Request):
    ...
    for item in items:
        if (item.get("position_barcode") == data.position_barcode or
            ...
            item["variance"] = data.counted_qty - item.get("system_qty", 0)
            item["variance_pct"] = (
                (item["variance"] / item["system_qty"] * 100) if item.get("system_qty", 0) > 0
                else (100.0 if data.counted_qty > 0 else 0.0)
            )
    ...
    return {"ok": True, "counted_items": counted_items, "total_items": s["total_items"]}
...
async def cancel_opname(session_id: str, request: Request, body: dict = {}):
    ...
    s = await db.wh_opname_sessions2.find_one({"id": session_id})
    if not s: raise HTTPException(404, "Sesi tidak ditemukan")
    if s["status"] == "approved": raise HTTPException(400, "Tidak dapat membatalkan sesi yang sudah approved")
    await db.wh_opname_sessions2.update_one({"id": session_id}, {"$set": {
...
async def count_sheet_pdf(session_id: str, request: Request, token: Optional[str] = None):
    ...
    s = await db.wh_opname_sessions2.find_one({"id": session_id}, {"_id": 0})
    if not s: raise HTTPException(404, "Sesi tidak ditemukan")
    if not REPORTLAB_OK: raise HTTPException(500, "ReportLab tidak tersedia")
    buf = io.BytesIO()
    W, H = A4
