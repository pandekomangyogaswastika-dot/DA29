...
async def create_grade(request: Request):
    ...
    if not code: raise HTTPException(400, "grade_code wajib.")
...
async def update_grade(grade_id: str, request: Request):
    ...
    if res.matched_count == 0: raise HTTPException(404, "Grade tidak ditemukan.")
...
async def audit(request: Request):
    ...
    employees = await db.rahaza_employees.find(
        {"active": True, "salary_grade_id": {"$exists": True, "$ne": None, "$ne": ""}},
