...
async def complaints_summary(request: Request):
    ...
    async for doc in db.marketing_complaints.aggregate(pipeline_status):  by_status[doc["_id"]]   = doc["count"]
    async for doc in db.marketing_complaints.aggregate(pipeline_sla):     by_sla[doc["_id"]]      = doc["count"]
    async for doc in db.marketing_complaints.aggregate(pipeline_cat):     by_cat[doc["_id"]]      = doc["count"]
    async for doc in db.marketing_complaints.aggregate(pipeline_plat):    by_platform[doc["_id"]] = doc["count"]
    async for doc in db.marketing_complaints.aggregate(pipeline_sev):     by_severity[doc["_id"]] = doc["count"]
...
async def list_complaints(
    ...
    if sla_status: q["sla_status"] = sla_status
    if category:   q["category"]   = category
    if severity:   q["severity"]   = severity
    if account_id: q["account_id"] = account_id
    if date_from or date_to:
        q["complaint_date"] = {}
        if date_from: q["complaint_date"]["$gte"] = datetime.fromisoformat(date_from)
        if date_to:   q["complaint_date"]["$lte"] = datetime.fromisoformat(date_to + "T23:59:59")
    if search:
        q["$or"] = [
