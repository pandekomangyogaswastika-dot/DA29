...
async def create_picklist(request: Request):
    ...
    if not items: raise HTTPException(400, "items wajib diisi.")
...
async def generate_from_source(source_type: str, source_id: str, request: Request):
    ...
    if source_type == "shipment":
        ...
        if not ship: raise HTTPException(404, "Shipment tidak ditemukan.")
        ...
        if not mi: raise HTTPException(404, "Material issue tidak ditemukan.")
        ...
        if not mv: raise HTTPException(404, "Pending movement tidak ditemukan.")
...
async def get_picklist(picklist_id: str, request: Request):
    ...
    if not doc: raise HTTPException(404, "Pick list tidak ditemukan.")
    return {"ok": True, "picklist": _s(doc)}
...
async def mark_item_picked(picklist_id: str, pick_item_id: str, request: Request):
    ...
    for it in items:
        if it.get("pick_item_id") == pick_item_id:
            ...
            found = True
            break
    if not found: raise HTTPException(404, "Pick item tidak ditemukan.")
...
async def complete_picklist(picklist_id: str, request: Request):
    ...
    if res.matched_count == 0: raise HTTPException(404, "Pick list tidak ditemukan.")
    return {"ok": True}
...
async def delete_picklist(picklist_id: str, request: Request):
    ...
    if res.deleted_count == 0: raise HTTPException(404, "Pick list tidak ditemukan.")
    return {"ok": True}
