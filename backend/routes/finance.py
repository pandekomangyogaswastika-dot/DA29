...
from routes.shared import new_id, now, parse_date, to_end_of_day, PO_STATUSES, get_pagination_params, paginated_response
import uuid
import json
import logging
...
async def get_invoices(request: Request):
    ...
    return serialize_doc(await db.invoices.find(query, {'_id': 0}).sort('created_at', -1).to_list(500))
...
async def get_invoice(inv_id: str, request: Request):
    ...
    payments = await db.payments.find({'invoice_id': inv_id}, {'_id': 0}).to_list(500)
    adjustments = await db.invoice_adjustments.find({'invoice_id': inv_id}, {'_id': 0}).sort('created_at', -1).to_list(500)
...
async def get_adjustments(request: Request):
    ...
    return serialize_doc(await db.invoice_adjustments.find({'invoice_id': inv_id}, {'_id': 0}).sort('created_at', -1).to_list(500))
...
async def create_adjustment(request: Request):
    ...
    all_adj = await db.invoice_adjustments.find({'invoice_id': body['invoice_id']}).to_list(500)
...
async def delete_adjustment(adj_id: str, request: Request):
    ...
    if invoice:
        all_adj = await db.invoice_adjustments.find({'invoice_id': doc['invoice_id']}).to_list(500)
        total_add = sum(a.get('amount', 0) for a in all_adj if a.get('adjustment_type') == 'ADD')
        total_deduct = sum(a.get('amount', 0) for a in all_adj if a.get('adjustment_type') == 'DEDUCT')
        base_amount = invoice.get('base_amount', invoice.get('total_amount', 0))
        new_total = base_amount + total_add - total_deduct
...
async def get_invoice_edit_requests(request: Request):
    ...
    if sp.get('q'):
        ...
        query['$or'] = [
            ...
            {'requested_by': {'$regex': re.escape(q), '$options': 'i'}},
            {'change_summary': {'$regex': re.escape(q), '$options': 'i'}}
        ]
    ...
    return serialize_doc(await db.invoice_edit_requests.find(query, {'_id': 0}).sort('requested_at', -1).to_list(500))
...
async def get_invoice_change_history(invoice_id: str, request: Request):
    ...
    db = get_db()
    history = await db.invoice_change_history.find({'invoice_id': invoice_id}, {'_id': 0}).sort('changed_at', -1).to_list(500)
...
async def get_payments(request: Request):
    ...
    if sp.get('page') or sp.get('limit'):
        ...
        items = await db.payments.find(query, {'_id': 0}).sort('payment_date', -1).skip(skip).limit(limit).to_list(limit)
        return paginated_response(serialize_doc(items), total, page, limit)
    return serialize_doc(await db.payments.find(query, {'_id': 0}).sort('payment_date', -1).to_list(500))
...
async def create_payment(request: Request):
    ...
    payment_date_iso = (body.get('payment_date') or now().date().isoformat())[:10]
    payment = {
        'id': new_id(), 'invoice_id': body['invoice_id'],
        'invoice_number': invoice.get('invoice_number'),
        ...
        'date': payment_date_iso,  # ISO date string for query convenience
    ...
    all_pmts = await db.payments.find({'invoice_id': body['invoice_id']}).to_list(500)
...
async def delete_payment(pay_id: str, request: Request):
    ...
    all_pmts = await db.payments.find({'invoice_id': doc.get('invoice_id')}).to_list(500)
    total_paid = sum(p.get('amount', 0) for p in all_pmts)
    invoice = await db.invoices.find_one({'id': doc.get('invoice_id')})
    if invoice:
...
async def accounts_payable(request: Request):
    await require_auth(request)
    db = get_db()
    return serialize_doc(await db.invoices.find({'invoice_category': 'VENDOR'}, {'_id': 0}).sort('created_at', -1).to_list(500))

@router.get("/accounts-receivable")
async def accounts_receivable(request: Request):
    await require_auth(request)
    db = get_db()
    return serialize_doc(await db.invoices.find({'invoice_category': 'BUYER'}, {'_id': 0}).sort('created_at', -1).to_list(500))
...
async def financial_recap(request: Request):
    ...
    invoices = await db.invoices.find(inv_query, {'_id': 0}).to_list(500)
    payments = await db.payments.find(pmt_query, {'_id': 0}).to_list(500)
    all_adj = await db.invoice_adjustments.find({}).to_list(500)
    adj_map = {}
    for adj in all_adj:
