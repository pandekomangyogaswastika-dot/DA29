    ...
    if new_status == "dispatched":
        ...
        try:
            ...
            if valid_items:
                ...
                wo_fg_codes = {}
                fg_codes = set()
                for wo_id, _ in valid_items:
                    wo_doc = wo_map.get(wo_id)
                    if not wo_doc: continue
                    m_doc = model_map.get(wo_doc.get("model_id"))
                    s_doc = size_map.get(wo_doc.get("size_id"))
                    if not m_doc or not s_doc: continue
                    code = f"FG-{m_doc['code']}-{s_doc['code']}"
                    wo_fg_codes[wo_id] = code
                    fg_codes.add(code)
                mat_map = {}
                if fg_codes:
                    async for m in db.rahaza_materials.find({"code": {"$in": list(fg_codes)}}, {"_id": 0}):
                        mat_map[m["code"]] = m

                from routes.wms_receiving import helper_create_pending_outbound_fg
                for wo_id, qty_shipped in valid_items:
                    wo_doc = wo_map.get(wo_id)
                    if not wo_doc: continue
                    fg_code = wo_fg_codes.get(wo_id)
                    if not fg_code: continue
                    mat_doc = mat_map.get(fg_code)
                    if not mat_doc: continue
                    await helper_create_pending_outbound_fg(
                        db,
                        material_id=mat_doc["id"],
                        material_code=fg_code,
                        material_name=mat_doc.get("name", fg_code),
                    ...
                    log.info(f"Pending OUTBOUND FG {qty_shipped} pcs for {fg_code} (Shipment {shp.get('shipment_number')})")
        except Exception as e:
