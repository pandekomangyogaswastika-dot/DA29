...
async def list_checklists(
    ...
    if q: filt["employee_name"] = {"$regex": re.escape(q), "$options": "i"}
...
async def create_checklist(
    ...
    if employee_id and not employee_name:
        ...
        if emp: employee_name = emp.get("name", employee_id)
    ...
    if template_id:
        ...
        if tpl: tasks = tpl.get("tasks", tasks)
...
async def get_checklist(
    ...
    if not doc: raise HTTPException(404, "Checklist tidak ditemukan")
    return {"ok": True, "checklist": serialize(doc)}
...
async def update_task(
    ...
    if not cl: raise HTTPException(404, "Checklist tidak ditemukan")
