...
async def list_overtime(
    ...
    if date_from or date_to:
        filt["date"] = {}
        if date_from: filt["date"]["$gte"] = date_from
        if date_to: filt["date"]["$lte"] = date_to
...
async def create_request(request: Request):
    ...
    if not date_str: raise HTTPException(400, "date wajib (YYYY-MM-DD).")
    start_time = body.get("start_time") or "17:00"
    end_time = body.get("end_time") or "19:00"
    hours = body.get("hours")
    if hours is None: hours = _calc_hours(start_time, end_time)
    if hours <= 0: raise HTTPException(400, "Durasi lembur tidak valid.")
...
async def approve_request(ot_id: str, request: Request):
    ...
    if not doc: raise HTTPException(404, "Overtime request tidak ditemukan.")
