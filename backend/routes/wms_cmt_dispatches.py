...
async def list_dispatches(
    ...
    if cmt_name: q["cmt_name"] = {"$regex": cmt_name, "$options": "i"}
    if wo_id: q["wo_id"] = wo_id
    if search:
        q["$or"] = [
...
async def get_dispatch(dispatch_id: str, request: Request):
    ...
    if not item: raise HTTPException(404, "Dispatch tidak ditemukan")
...
async def update_dispatch(dispatch_id: str, data: dict, request: Request):
    ...
    if not item: raise HTTPException(404, "Dispatch tidak ditemukan")
    if item["status"] != "draft": raise HTTPException(400, "Hanya draft yang dapat diupdate")
...
async def execute_dispatch(dispatch_id: str, request: Request, body: dict = {}):
    ...
    item = await db.wh_cmt_dispatches.find_one({"id": dispatch_id})
    if not item: raise HTTPException(404, "Dispatch tidak ditemukan")
    if item["status"] != "draft": raise HTTPException(400, "Hanya draft yang dapat di-dispatch")
    if not item.get("lines"): raise HTTPException(400, "Minimal 1 item diperlukan")
...
async def return_materials(dispatch_id: str, data: ReturnLineIn, request: Request):
    ...
    for ln in lines:
        if ln.get("material_code") == data.material_code:
            ...
            updated = True
            break
    if not updated: raise HTTPException(404, f"Material {data.material_code} tidak ada di dispatch")
...
async def cancel_dispatch(dispatch_id: str, request: Request, body: dict = {}):
    ...
    item = await db.wh_cmt_dispatches.find_one({"id": dispatch_id})
    if not item: raise HTTPException(404, "Dispatch tidak ditemukan")
    if item["status"] in ("fully_returned",): raise HTTPException(400, "Tidak dapat membatalkan yang sudah selesai")
    await db.wh_cmt_dispatches.update_one({"id": dispatch_id}, {"$set": {
