...
async def job_scan_overdue_invoices():
    ...
    try:
        ...
        queued = 0
        for inv in invoices:
            existing = await db.dewi_notifications.find_one({
...
async def job_birthday_anniversary_reminders():
    ...
    try:
        ...
        # Queue notifications to HR admins
        created = 0
        if bdays or annivs:
            try:
                from routes.rahaza_notifications import _queue_notif  # if available
            except ImportError:
                _queue_notif = None
            ...
            for bd in bdays:
                ...
                for hr in hr_users:
                    await db.rahaza_notifications.insert_one({
                        'id': str(started.timestamp()) + '_' + hr['id'] + '_bd_' + bd['code'],
                        'user_id': hr['id'], 'type': 'birthday',
                        'title': title, 'body': body_msg,
                        'is_read': False, 'created_at': started,
                    })
                    created += 1
            for av in annivs:
                title = f"🎉 Work Anniversary: {av['name']} ({av['years']} tahun)"
                body_msg = f"{av['code']} — {av['name']} merayakan {av['years']} tahun bekerja. Berikan apresiasi!"
                for hr in hr_users:
                    await db.rahaza_notifications.insert_one({
                        'id': str(started.timestamp()) + '_' + hr['id'] + '_anv_' + av['code'],
                        'user_id': hr['id'], 'type': 'anniversary',
                        'title': title, 'body': body_msg,
                        'is_read': False, 'created_at': started,
                    })
                    created += 1
