...
class TokoMigrationTester:
        ...
        if success:
            ...
            try:
                ...
                if mirror_doc is None:
                    self.log(f"   ✓ Mirror deleted from marketing_catalog_items")
                    return True
                else:
                    self.log(f"   ❌ Mirror still exists in marketing_catalog_items", "FAIL")
                    return False
            except Exception as e:
    ...
    def test_list_products(self) -> bool:
        ...
        if success and isinstance(response, list):
            ...
            self.log(f"   ✓ List endpoint works (deprecated but functional)")
            return True
        return False
    ...
    async def test_list_channels(self) -> bool:
        ...
        if success and isinstance(response, list):
            ...
            if mirror_count >= 4:
                self.log(f"   ✓ All 4 channels mirrored to marketing_platform_accounts")
                return True
            else:
    ...
    async def test_update_channel(self) -> bool:
        ...
        if success:
            ...
            try:
                ...
                if mirror_doc:
                    self.log(f"   ✓ Channel mirror found in marketing_platform_accounts")
                    return True
                else:
                    self.log(f"   ❌ Channel mirror NOT found", "FAIL")
                    return False
            except Exception as e:
        ...
        if success:
            ...
            try:
                ...
                if sync_mirror:
                    ...
                    self.log(f"   ❌ Sync log NOT mirrored", "FAIL")
                    return False
            except Exception as e:
        ...
        if success:
            ...
            try:
                ...
                if mirror_doc and mirror_doc.get("packed_date"):
                    self.log(f"   ✓ packed_date populated in mirror")
                    return mirror_verified
                else:
                    self.log(f"   ❌ packed_date NOT populated", "FAIL")
                    return False
            except Exception as e:
            ...
            try:
                ...
                if mirror_doc:
                    ...
                    if status_ok and tracking_ok and shipped_date_ok:
                        self.log(f"   ✓ Mirror updated: status=shipped, tracking_number=ABC123, shipped_date populated")
                        return True
                    else:
                        self.log(f"   ❌ Mirror incomplete: status={status_ok}, tracking={tracking_ok}, shipped_date={shipped_date_ok}", "FAIL")
                        return False
                else:
                    self.log(f"   ❌ Mirror NOT found", "FAIL")
                    return False
            except Exception as e:
        ...
        if success and 'id' in response:
            ...
            try:
                ...
                if mirror_doc and mirror_doc.get("return_number"):
                    ...
                    self.log(f"   ❌ return_number NOT generated", "FAIL")
                    return False
            except Exception as e:
            ...
            try:
                ...
                if mirror_doc:
                    ...
                    if response_ok and status_ok:
                        self.log(f"   ✓ Mirror updated: response_text set, status='responded'")
                        return True
                    else:
    ...
    def test_migration_idempotency(self) -> bool:
        ...
        try:
            ...
            if "skipped_existing" in output:
                self.log(f"   ✓ Migration idempotent: re-run skipped existing records")
                return True
            else:
                self.log(f"   ⚠ Migration output unexpected (no skipped_existing found)", "WARN")
