...
async def main() -> int:
    ...
    print(f"[PASS] US1: Seeded 2 legacy orders")

    # US2: Convert order1 via adapter
    payload = order_to_po_create_payload(legacy_orders[0])
    assert payload["id"] == order1_id, f"US2 FAILED: id mismatch"
    ...
    print(f"[PASS] US2: adapter converts order with qty_per_size to multi-item PO")
    ...
    print(f"[PASS] US3: adapter handles single-item order with linked_wo_ids")
    ...
    print(f"[PASS] US4: 2 POs inserted to dewi_maklon_pos")
    ...
    print(f"[PASS] US5: adapter round-trip preserves data shape")
    ...
    results.append(("US6: find_maklon_record() resolves id + po_number", True,
                    f"both resolved from dewi_maklon_pos"))
    print(f"[PASS] US6: find_maklon_record() resolves from both id & po_number")
