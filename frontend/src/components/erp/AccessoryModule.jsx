...
import {
  ...
  ChevronDown, ChevronUp, RefreshCw, Eye, Check, XCircle,
  ArrowUpCircle, ArrowDownCircle, ClipboardCheck, Banknote,
...
function MasterTab({ token, onRefreshDash }) {
  ...
  const [form, setForm]         = useState({ code:'', name:'', category:'Umum', unit:'pcs', description:'', min_stock:0, supplier:'', notes:'' });
  const [showMove, setShowMove] = useState(null); // {id, name, action:'in'|'out'}
  const [moveForm, setMoveForm] = useState({ qty:'', notes:'' });
  ...
  const openAdd = () => { setEditItem(null); setForm({ code:'', name:'', category:'Umum', unit:'pcs', description:'', min_stock:0, supplier:'', notes:'' }); setShowForm(true); };
  const openEdit = it => { setEditItem(it); setForm({ code: it.code||'', name: it.name||'', category: it.category||'Umum', unit: it.unit||'pcs', description: it.description||'', min_stock: it.min_stock||0, supplier: it.supplier||'', notes: it.notes||'' }); setShowForm(true); };
  ...
  const doMove = async () => {
    ...
    try {
      ...
      await api('POST', path, token, { acc_id: showMove.id, qty, notes: moveForm.notes });
      setShowMove(null); setMoveForm({ qty:'', notes:'' }); load(); onRefreshDash();
    } catch(e) { setErr(e.message); }
    finally { setSaving(false); }
  };
        ...
        <table className="w-full text-sm min-w-[700px]">
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan="7" className="text-center py-8 text-muted-foreground">Belum ada data</td></tr>
            ) : items.map(it => (
              <tr key={it.id} className="border-b border-border hover:bg-white/2" data-testid={`acc-row-${it.id}`}>
                ...
                <td className="px-4 py-3 text-right font-medium">{fmtNum(it.stock_qty)} <span className="text-xs text-muted-foreground">{it.unit}</span></td>
                <td className="px-4 py-3 text-right text-muted-foreground text-xs">{fmtNum(it.min_stock)}</td>
                <td className="px-4 py-3 text-center">
                  <Badge status={it.stock_status} label={it.stock_status==='ok'?'Aman':it.stock_status==='low'?'Rendah':'Habis'} />
                </td>
                <td className="px-4 py-3">
                  <div className="flex items-center justify-end gap-1">
                    <button onClick={()=>{setShowMove({id:it.id,name:it.name,action:'in'}); setMoveForm({qty:'',notes:''}); setErr('');}}
                      className="p-1 hover:bg-emerald-500/10 rounded" title="Terima Stok" data-testid={`acc-in-${it.id}`}>
                      <PackagePlus className="w-4 h-4 text-emerald-400" />
                    </button>
                    <button onClick={()=>{setShowMove({id:it.id,name:it.name,action:'out'}); setMoveForm({qty:'',notes:''}); setErr('');}}
      ...
      {showMove && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={()=>setShowMove(null)}>
          <div className="bg-[var(--card-surface)] rounded-2xl shadow-xl w-full max-w-sm p-6" onClick={e=>e.stopPropagation()}>
            ...
            <div className="space-y-3">
              <div>
                <label className="text-xs text-muted-foreground block mb-1">Jumlah *</label>
      ...
      <div className="bg-[var(--card-surface)] rounded-xl border border-border overflow-x-auto">
        <table className="w-full text-sm min-w-[650px]">
          ...
          <tbody>
            ...
            : requests.length === 0 ? <tr><td colSpan="7" className="text-center py-8 text-muted-foreground">Belum ada request</td></tr>
            : requests.map(r => (
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="space-y-3">
        ...
        : sessions.length === 0 ? <div className="text-center py-8 text-muted-foreground">Belum ada sesi opname</div>
        : sessions.map(s => (
      ...
      <div className="bg-[var(--card-surface)] rounded-xl border border-border overflow-x-auto">
        <table className="w-full text-sm min-w-[700px]">
          ...
          <tbody>
            ...
            : loans.length === 0 ? <tr><td colSpan="8" className="text-center py-8 text-muted-foreground">Belum ada peminjaman</td></tr>
            : loans.map(l => (
    ...
    <div className="space-y-5">
      ...
      <div className="bg-[var(--card-surface)] rounded-xl border border-border overflow-x-auto">
        <table className="w-full text-sm min-w-[700px]">
          ...
          <tbody>
            {loading ? <tr><td colSpan="7" className="text-center py-8 text-muted-foreground">Memuat...</td></tr>
            : prs.length === 0 ? <tr><td colSpan="7" className="text-center py-8 text-muted-foreground">Belum ada purchase request</td></tr>
            : prs.map(pr => (
