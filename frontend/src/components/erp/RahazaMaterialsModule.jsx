...
export default function RahazaMaterialsModule({ token }) {
  ...
  const [form, setForm] = useState({
    ...
    min_stock: 0, min_stock_qty: '', min_stock_percentage: '', active: true,
  });
  ...
  const openCreate = () => {
    setEditing(null);
    setForm({ code: '', name: '', type: 'yarn', unit: 'kg', yarn_type: '', color: '', notes: '', min_stock: 0, min_stock_qty: '', min_stock_percentage: '', active: true });
    setFormError(''); setModalOpen(true);
  };
  const openEdit = (r) => {
    setEditing(r);
    setForm({ ...r, min_stock_qty: r.min_stock_qty || '', min_stock_percentage: r.min_stock_percentage || '' });
    setFormError(''); setModalOpen(true);
  };
  const save = async () => {
