...
export default function AccessoryRequestInbox({ token }) {
  ...
  useEffect(() => { fetchRequests(); }, [filterStatus, urgentOnly]); // eslint-disable-line react-hooks/exhaustive-deps
    ...
    <div className="p-6 space-y-5" data-testid="accessory-request-inbox">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-foreground/50 mt-0.5">Permintaan aksesoris dari tim RnD untuk produksi sample</p>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-3 items-center">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <Input value={search} onChange={e => setSearch(e.target.value)}
            ...
            placeholder="Cari kode / style / requester..." className="pl-9"
      ...
      {loading ? (
        ...
        <GlassCard className="p-10 text-center">
          ...
          <p className="text-foreground/50 text-sm">Tidak ada request aksesoris.</p>
        </GlassCard>
      ) : (
        <div className="overflow-x-auto rounded-xl border border-white/10">
          <table className="w-full text-sm">
            <thead className="bg-white/5 border-b border-white/10">
              <tr>
                {['Kode', 'Style', 'Items', 'Butuh Tgl', 'Status', 'Requester', 'Tgl Buat', 'Aksi'].map(h => (
        ...
        <Modal onClose={() => setDetail(null)} title={`Detail: ${detail.request_code}`} size="lg">
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div>
                <div className="text-xs text-foreground/50">Urgent</div>
                <div>{detail.urgent ? '🚨 Ya' : 'Tidak'}</div>
              </div>
            </div>
            <div>
