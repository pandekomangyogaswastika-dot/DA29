  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Hutang (AP)</h1>
          <p className="text-muted-foreground text-sm mt-1">Kelola tagihan dari supplier/pemasok. Invoice dapat dibuat otomatis atau manual.</p>
        </div>
        <button onClick={fetchInvoices} data-testid="ap-refresh" className="flex items-center gap-2 px-3 py-2 border border-border rounded-lg text-sm hover:bg-[var(--glass-bg)]">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Total Tagihan', value: fmt(totalAll), cls: 'text-foreground', bg: 'bg-[var(--glass-bg)] border-border', icon: <FileText className="w-5 h-5 text-muted-foreground" /> },
      ...
      <div className="space-y-2">
        ...
        <div className="flex flex-wrap gap-2 items-center">
          ...
          {FILTER_OPTIONS.map(opt => (
            <button key={opt.value} onClick={() => setDateFilter(opt.value)} data-testid={`ap-filter-${opt.value}`}
        ...
        <div className="flex gap-2 flex-wrap">
          {STATUS_OPTS.map(s => (
            <button key={s} onClick={() => setFilterStatus(s)}
              className={`px-3 py-1.5 rounded-lg text-sm border transition-colors ${filterStatus === s ? 'bg-amber-600 text-white border-amber-600' : 'border-border text-muted-foreground hover:bg-[var(--glass-bg)]'}`}>
              {s || 'Semua Status'}
            </button>
          ))}
      ...
      {loading ? (
        ...
        <div className="text-center py-16">
          ...
          <p className="text-muted-foreground font-medium">Belum ada invoice hutang</p>
          <p className="text-muted-foreground text-sm mt-1">Invoice dapat dibuat otomatis saat penerimaan barang, atau dibuat manual.</p>
        </div>
      ) : (
                ...
                return (
                  <tr key={inv.id} className="hover:bg-[var(--glass-bg)]">
                    ...
                    <td className="px-4 py-3">
                      <p className="font-medium text-foreground">{inv.vendor_name || inv.garment_name || inv.vendor_or_customer_name || '-'}</p>
                    </td>
                    <td className="px-4 py-3 text-xs text-muted-foreground font-mono">{inv.shipment_number || inv.po_number || '-'}</td>
          ...
          <form onSubmit={handlePayment} className="space-y-4">
            <div className="bg-[var(--glass-bg)] rounded-xl p-4">
              <div className="grid grid-cols-3 gap-3 text-sm">
                <div><p className="text-xs text-muted-foreground">Vendor</p><p className="font-medium">{selectedInvoice.vendor_name || selectedInvoice.garment_name}</p></div>
                ...
                <div><p className="text-xs text-muted-foreground">Sisa Belum Bayar</p><p className="font-bold text-red-600">{fmt((selectedInvoice.total_amount || 0) - (selectedInvoice.total_paid || 0))}</p></div>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-foreground mb-1">Tanggal Bayar *</label>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-foreground mb-1">Metode Bayar</label>
